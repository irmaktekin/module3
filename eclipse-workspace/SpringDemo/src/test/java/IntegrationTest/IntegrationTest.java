...
public void setup() {
	...
	storage = appContext.getBean(Storage.class);
...
public void createUserAndBookTicket() {
	...
	assertEquals(newEvent, storage.getById("event"+newEvent.getId()));
	assertEquals(ticket, storage.getById("ticket"+ticket.getId()));
